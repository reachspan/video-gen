...
| `T14` | **Beauty bias / type collapse.** Faces snap to a type — handsome-and-young, or weathered-and-irregular — instead of the person asked for. Worst under text-only identity. | only visible across generations: several seeds resemble each other more than several real people would |
...
instruction to the contrary. Reported against Seedance before 2.5; not re-measured
on it. Confirm a default that stops showing up rather than treating it as an
observation error — each is still cheap to write against, so none is worth dropping
on a suspicion.
